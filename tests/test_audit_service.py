...
from app.models.schemas import PackageInfo, AuditResult
...
def mock_pypi_client():
    client = MagicMock()
    client.get_package_info = AsyncMock(return_value={
        "version": "1.0.0",
...
async def test_run_audit_flow_parse_error(audit_service):
    with patch("app.services.parser.parse_requirements", side_effect=Exception("Parse Error")):
        with pytest.raises(ValueError, match="檔案解析失敗"):
            await audit_service.run_audit_flow(b"invalid", "reqs.txt")

@pytest.mark.asyncio
async def test_run_audit_flow_no_packages(audit_service):
    with patch("app.services.parser.parse_requirements", return_value=[]):
        with pytest.raises(ValueError, match="未解析到任何套件"):
